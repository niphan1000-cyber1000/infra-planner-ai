...
import { Flame, Play, TrendingUp, CheckCircle, Loader2 } from "lucide-react";
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-white/10 pb-4 mb-5 gap-3">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            ทดสอบสถานการณ์วิกฤต เช่น Flash Sale ทะลัก หรือ DDoS โจมตี
            เพื่อประเมินว่าสถาปัตยกรรมที่ออกแบบจะต้านทานไหวหรือไม่
          </p>
        </div>
          ...
          {[
            {
              id: "normal",
              label: "🟢 ปกติ",
              color: "hover:bg-emerald-500/10 hover:border-emerald-500",
            },
            {
              id: "spike",
              label: "⚡ Flash Sale (+500%)",
              color: "hover:bg-amber-500/10 hover:border-amber-500",
            },
            {
              id: "ddos",
              label: "💀 DDoS Attack",
            ...
            {
              id: "hybrid_fail",
              label: "📡 เน็ตหน่วงข้ามคลาวด์",
              color: "hover:bg-indigo-500/10 hover:border-indigo-500",
            },
            {
              id: "db_lock",
              label: "🔒 DB เกิดเดดล็อก",
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-4 flex flex-col gap-3">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">
              สถานะโทรมาตรเสมือน (Telemetry metrics)
            </span>
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <div className="mt-2 flex items-baseline justify-between">
                  <span
                    className={`text-xl font-bold ${
                      simResult.metrics.latency > 150
                        ? "text-red-400"
                        : simResult.metrics.latency > 50
                          ? "text-amber-400"
                          : "text-emerald-400"
                    }`}
                  >
              ...
              <div className="bg-[#05070a] border border-white/10 rounded-xl p-3.5 flex flex-col justify-between">
                ...
                <div className="mt-2 flex items-baseline justify-between">
                  <span className="text-xl font-bold text-white">
                    {simResult.metrics.throughput.toLocaleString()}{" "}
                    <span className="text-xs font-normal">req/s</span>
                  </span>
              ...
              <div className="bg-[#05070a] border border-white/10 rounded-xl p-3.5 flex flex-col justify-between">
                ...
                <div className="mt-2 flex items-baseline justify-between">
                  <span
                    className={`text-xl font-bold ${
                      simResult.metrics.cpuLoad > 85
                        ? "text-red-400"
                        : simResult.metrics.cpuLoad > 60
                          ? "text-amber-400"
                          : "text-emerald-400"
                    }`}
                  >
                ...
                <div className="w-full bg-white/5 h-1.5 rounded-full mt-2 overflow-hidden">
                  <div
                    ...
                    style={{ width: `${simResult.metrics.cpuLoad}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div className="bg-[#05070a] border border-white/10 rounded-xl p-3.5 flex flex-col justify-between">
                ...
                <div className="mt-2 flex items-baseline justify-between">
                  <span
                    className={`text-xl font-bold ${
                      simResult.metrics.dbConnections > 300
                        ? "text-red-400"
                        : simResult.metrics.dbConnections > 150
                          ? "text-amber-400"
                          : "text-white"
                    }`}
                  >
                    {simResult.metrics.dbConnections}{" "}
                    <span className="text-xs font-normal">/ 500</span>
                  </span>
            ...
            <div
              className={`p-4 rounded-xl border flex items-center justify-between ${
                simResult.systemStatus === "healthy"
                  ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-300"
                  : simResult.systemStatus === "degraded"
            ...
            >
              <div>
                <p className="text-[10px] uppercase font-bold tracking-wider">
                  ภาพรวมความต้านทานระบบ
                </p>
                <h4 className="text-sm font-extrabold mt-0.5">
                  {simResult.systemStatus === "healthy"
                    ? "✅ ปลอดภัยดีเยี่ยม (Optimal Status)"
                    : simResult.systemStatus === "degraded"
                      ? "⚠️ ระบบล่าช้าเล็กน้อย (Performance Degraded)"
          ...
          <div className="lg:col-span-4 flex flex-col gap-2.5">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">
              เหตุการณ์จำลองเสมือน (Real-time Simulation Logs)
            </span>
                ...
                <div
                  ...
                  className={`leading-relaxed border-l-2 pl-2 ${
                    line.includes("[CRITICAL]")
                      ...
                      : line.includes("[WARN]") || line.includes("[ALERT]")
          ...
          <div className="lg:col-span-4 flex flex-col gap-2.5">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">
              มาตรการแก้ไขและบรรเทาทันที (Proactive AI Recommendation)
            </span>
            ...
            <div className="flex-1 bg-indigo-950/15 border border-indigo-500/20 rounded-xl p-4 flex flex-col justify-between">
              <div>
                {simResult.bottlenecksTriggered.length > 0 ? (
                  <div className="mb-3">
                    <span className="text-[9px] font-extrabold text-red-400 uppercase tracking-wider block mb-1">
                      คอขวดที่ตรวจจับเจอ:
                    </span>
                    <div className="flex flex-wrap gap-1">
                      {simResult.bottlenecksTriggered.map((b) => (
                        <span
                          key={b}
                          className="text-[9px] font-mono bg-red-500/10 border border-red-500/30 text-red-300 px-1.5 py-0.5 rounded"
                        >
                          {b}
                        </span>
                      ))}
                  ...
                  <div className="flex items-center gap-1 text-[11px] text-emerald-400 font-bold mb-3">
                    ...
                    ตรวจไม่พบปัญหาคอขวดที่รุนแรงในโครงสร้างปัจจุบัน!
                  </div>
                )}

                <span className="text-[10px] font-bold text-slate-400 block mb-1.5">
                  ขั้นตอนสลายคอขวดที่แนะนำ:
                </span>
                <ul className="text-xs text-slate-300 space-y-1.5 list-disc pl-4 leading-relaxed">
                  {simResult.recommendations.length > 0 ? (
                    simResult.recommendations.map((rec, idx) => <li key={idx}>{rec}</li>)
                  ) : (
                    <li>
                      โครงสร้างสถาปัตยกรรมที่ออกแบบทนทานแรงกดดันได้ดีมากในสเกลปัจจุบัน
                      แนะนำให้หมั่นตรวจสอบ Log และตั้งงบประมาณอย่างรัดกุม
                    </li>
                  )}
                </ul>
              </div>
              ...
              <button
                onClick={() => {
                  ...
                  dispatch({
                    ...
                    value: `ช่วยอธิบายวิธีแก้ไขเรื่อง "${simResult.bottlenecksTriggered[0] || "การรับส่งข้อมูลและการขยายตัว"}" อย่างละเอียด และขอแนวทางการคอนฟิกสตรีมมิ่งด้วยครับ`,
                  });
                }}
