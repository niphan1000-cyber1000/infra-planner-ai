  ...
  while (attempt < maxRetries) {
    try {
      ...
      if (!isTransientError(err)) {
        logger.warn(
          `Non-transient error detected inside executeWithRetry. Failing fast:`,
          err.message || err
        );
        throw err;
      }
      ...
      logger.warn(
        `Gemini API call transient failure, retrying attempt ${attempt}/${maxRetries} in ${delay}ms...`,
        err
      );
...
const responseSchema = {
  ...
  properties: {
    ...
    architectureStyle: {
      type: Type.STRING,
      description:
        "รูปแบบสถาปัตยกรรมหลัก เช่น Microservices, Event-Driven, Hybrid Cloud, Serverless, Monolithic Modernization",
    },
    hybridCloudStrategy: {
      type: Type.STRING,
      description:
        "แนวทางการบูรณาการกลยุทธ์ Hybrid Cloud หรือการรวมเทคโนโลยี On-Premise และ Cloud เข้าด้วยกันอย่างมีประสิทธิภาพ ภาษาไทย",
    },
    systemAnalysis: {
      type: Type.OBJECT,
    ...
    itStrategyRoadmap: {
      ...
      properties: {
        phase1ShortTerm: {
          type: Type.STRING,
          description:
            "แผนระยะสั้น (0 - 6 เดือน): Phase 1 การเตรียมความพร้อมและปรับเปลี่ยนโครงสร้างหลัก ภาษาไทย",
        },
        phase2MidTerm: {
          type: Type.STRING,
        ...
        phase3LongTerm: {
          type: Type.STRING,
          description:
            "แผนระยะยาว (18+ เดือน): Phase 3 สู่ระบบคลาวด์และสถาปัตยกรรมแบบ Serverless เสถียรภาพสูงสุด ภาษาไทย",
        },
        futureGrowthAdaptability: {
          type: Type.STRING,
          description:
            "การเตรียมพร้อมระบบเพื่อรองรับการเปลี่ยนแปลงทางธุรกิจในอนาคต 3-5 ปีข้างหน้าอย่างยืดหยุ่น ภาษาไทย",
        },
      },
    ...
    riskManagementPlan: {
      ...
      properties: {
        ...
        threatMitigationControls: {
          type: Type.STRING,
          description:
            "มาตรการควบคุมเพื่อบรรเทาภัยคุกคามทางไซเบอร์และความเป็นส่วนตัวของข้อมูล ภาษาไทย",
        },
        businessContinuityPlan: {
          type: Type.STRING,
          description:
            "แผนสำรองข้อมูลกู้คืนภัยพิบัติ DR Plan กำหนดเป้าหมาย RTO/RPO เพื่อความต่อเนื่องทางธุรกิจ ภาษาไทย",
        },
      },
    ...
    nodes: {
      type: Type.ARRAY,
      description:
        "รายการองค์ประกอบหรือระบบย่อยในผังโครงสร้าง (Topology Nodes) มีอย่างน้อย 6-9 โหนดเพื่อแสดงความครบถ้วนของระบบระดับ Enterprise",
      items: {
        type: Type.OBJECT,
        properties: {
          id: {
            type: Type.STRING,
            description: "รหัสเฉพาะของโหนด เช่น web_app, database, api_gateway",
          },
          name: {
            type: Type.STRING,
            description: "ชื่อโหนดภาษาไทย เช่น เว็บแอปพลิเคชัน, ระบบฐานข้อมูลหลัก",
          },
          category: {
            type: Type.STRING,
            description:
              "ประเภทโหนด: ingress, security, gateway, compute, cache, database, queue, integration, other",
          },
          serviceName: {
            type: Type.STRING,
            description: "ชื่อบริการจริง เช่น Amazon ECS, Azure SQL, On-Premises Oracle",
          },
          details: {
            type: Type.STRING,
            description: "รายละเอียดสเปค เช่น Multi-AZ, Read Replicas, Auto-scaling",
          },
          status: {
            type: Type.STRING,
            description: "สถานะความมั่นคงปลอดภัย: secure, warning, critical",
          },
          provider: {
            type: Type.STRING,
            description: "ผู้ให้บริการ: aws, azure, gcp, hybrid, on-premise",
          },
          description: {
            type: Type.STRING,
            description: "อธิบายสั้นๆ เกี่ยวกับหน้าที่ของโหนดนี้ในระบบ",
          },
        },
        required: [
          "id",
    ...
    connections: {
      type: Type.ARRAY,
      description:
        "รายการเส้นทางการเชื่อมโยงและการส่งข้อมูลระหว่างโหนดต่างๆ จากผู้ใช้ปลายทางไปจนถึงระบบฐานข้อมูลและ On-premise",
      items: {
        type: Type.OBJECT,
        properties: {
          ...
          label: {
            type: Type.STRING,
            description:
              "คำอธิบายเส้นทางไหลข้อมูล เช่น HTTPS (TLS 1.3), REST API, gRPC, CDC Async Sync, JDBC/TCP",
          },
        },
    ...
    bottlenecks: {
      ...
      items: {
        ...
        properties: {
          ...
          description: {
            type: Type.STRING,
            description: "รายละเอียดและปัจจัยกระตุ้นให้เกิดปัญหานั้นๆ ภาษาไทย",
          },
          severity: { type: Type.STRING, description: "ระดับความรุนแรง: high, medium, low" },
          solution: {
            type: Type.STRING,
            description: "แผนหรือเทคนิคการสลายคอขวดอย่างละเอียด ภาษาไทย",
          },
        },
    ...
    securityRisks: {
      ...
      items: {
        ...
        properties: {
          ...
          description: {
            type: Type.STRING,
            description: "รายละเอียดความเสี่ยง วิธีที่ผู้โจมตีอาจทำ หรือช่องโหว่ทางเทคนิค ภาษาไทย",
          },
          severity: { type: Type.STRING, description: "ระดับความรุนแรง: high, medium, low" },
          mitigation: {
            type: Type.STRING,
            description: "มาตรการแก้ไขและตั้งค่าระบบไอทีเชิงรับเชิงรุก ภาษาไทย",
          },
        },
      ...
      items: {
        ...
        properties: {
          item: {
            type: Type.STRING,
            description:
              "รายการวิธีการประหยัด เช่น Instance Savings Plans, Auto-shutdown dev servers, Spot Instances, Hybrid Database offloading",
          },
          currentEstimate: {
            type: Type.STRING,
            description:
              "ค่าใช้จ่ายโดยประมาณต่อเดือนในกรณีปกติ (ระบุเป็นดอลลาร์สหรัฐหรือบาทต่อเดือน)",
          },
          hybridStrategy: {
            type: Type.STRING,
            description: "กลยุทธ์ไฮบริดหรือเครื่องมือประหยัดเพิ่มเติม ภาษาไทย",
          },
          potentialSavings: {
            type: Type.STRING,
            description: "มูลค่าหรือสัดส่วนที่คาดว่าจะประหยัดได้ เช่น 30% หรือ $150 ต่อเดือน",
          },
        },
      ...
      items: {
        ...
        properties: {
          category: {
            type: Type.STRING,
            description:
              "ประเภทเทคโนโลยี เช่น Compute, Database, Security, Network, Logging/Monitoring",
          },
          awsProduct: {
            type: Type.STRING,
            description: "บริการหรือสินค้าเด่นฝั่ง AWS เช่น Amazon ECS, RDS, CloudWatch",
          },
          azureProduct: {
            type: Type.STRING,
            description:
              "บริการหรือสินค้าเด่นฝั่ง Azure เช่น Azure Container Apps, SQL Database, Monitor",
          },
          gcpProduct: {
            type: Type.STRING,
            description: "บริการหรือสินค้าเด่นฝั่ง GCP เช่น Cloud Run, Cloud SQL, Cloud Logging",
          },
          hybridApproach: {
            type: Type.STRING,
            description:
              "แนวทางการทำไฮบริดหรือซิงก์ข้อมูลระหว่างคลาวด์และ On-Premise เช่น Anthos, Arc, Outposts, Kubernetes",
          },
          pros: {
            type: Type.STRING,
            description: "ข้อดีหลักของการเปรียบเทียบและการเลือกสถาปัตยกรรมนี้ ภาษาไทย",
          },
...
const getFallbackArchitectureReport = (payload: any) => {
  return {
    ...
    hybridCloudStrategy:
      "เชื่อมต่อ On-Premise Data Center และ Cloud ด้วยระบบไฮบริด VPN / Direct Connect ที่มีความปลอดภัยระดับสูงตามมาตรฐานสากล",
    systemAnalysis: {
      ...
      improvementPath:
        "1. ติดตั้งระบบคัดกรองการเข้าถึงหน้าบ้านผ่าน Web Application Firewall\n2. นำ API Gateway มาช่วยจัดการสิทธิ์การเข้าใช้งาน (Authentication/Authorization)\n3. ใช้ระบบ In-Memory caching (Redis) เพื่อสลายคอขวดดึงข้อมูลซ้ำซ้อน\n4. เชื่อมโยงระบบฐานข้อมูลแบบจัดสรร Master-Slave เพื่อความมั่นคงสูงสุด",
      businessGoalAlignment:
        "การออกแบบคำนึงถึงเป้าหมายและงบประมาณเพื่อควบคุมค่าใช้จ่าย พร้อมรองรับการเติบโตแบบยืดหยุ่น",
    },
    itStrategyRoadmap: {
      phase1ShortTerm:
        "จัดสรรสิทธิ์และตั้งค่าโครงสร้างพื้นฐาน (Infrastructure baseline setup) และเปิดใช้ด่านความปลอดภัย",
      phase2MidTerm:
        "เริ่มทยอยทำ Data migration และทดลองรันงานบางส่วนบนสภาพแวดล้อมระบบคลาวด์ควบคู่กัน",
      phase3LongTerm:
        "ทำการย้ายบริการประมวลผลหลักขึ้นสู่สถาปัตยกรรมไร้เซิร์ฟเวอร์แบบ Hybrid เต็มรูปแบบพร้อมลดทอนระบบเก่า",
      futureGrowthAdaptability:
        "สถาปัตยกรรมสามารถปรับเปลี่ยนหรือติดตั้งบริการเสริมอื่นๆ เพิ่มเติมได้ในภายหลังอย่างไม่จำกัด",
    },
    riskManagementPlan: {
      riskIdentification:
        "ความเสี่ยงด้านเวลาในการเชื่อมโยงและการสูญหายของแพ็กเก็ตข้อมูลระหว่างระบบ On-Premise และ Cloud",
      threatMitigationControls:
        "การเข้ารหัสข้อมูลทุกระดับ (Encryption at rest & in transit) และบังคับใช้ระบบยืนยันตัวตนแบบหลายปัจจัย (MFA)",
      businessContinuityPlan:
        "จัดทำแผนสำรองความเสี่ยงฉุกเฉย (Disaster Recovery Plan) โดยตั้งค่า RTO ต่ำกว่า 4 ชั่วโมง และ RPO ต่ำกว่า 1 ชั่วโมง",
    },
    nodes: [
      {
        id: "ingress",
        name: "ระบบไฟร์วอลล์ด่านหน้า (WAF/CloudFront)",
        category: "security",
        serviceName: "Cloud Web Application Firewall",
        details: "DDoS Mitigation Layer",
        status: "secure",
        provider:
          payload.cloudPreference === "hybrid" || payload.cloudPreference === "on-premise"
            ? "gcp"
            : payload.cloudPreference,
        description: "ด่านป้องกันภัยคุกคามทางไซเบอร์และการส่งทราฟฟิกเข้ามาในระบบ",
      },
      {
        id: "api_gateway",
        name: "ระบบคัดกรองคำขอ (API Gateway)",
        category: "gateway",
        serviceName: "Enterprise API Gateway",
        details: "Rate Limiting & Authentication Active",
        status: "secure",
        provider:
          payload.cloudPreference === "hybrid" || payload.cloudPreference === "on-premise"
            ? "gcp"
            : payload.cloudPreference,
        description: "ศูนย์กลางรับส่งสัญญาณ API และสกรีนสิทธิ์การเข้าใช้งาน",
      },
      {
        id: "compute_app",
        name: "ระบบประมวลผลเซิร์ฟเวอร์หลัก (Compute Node)",
        category: "compute",
        serviceName: "Kubernetes Cluster / App Service",
        details: "Auto-scaling enabled (CPU target 70%)",
        status: "secure",
        provider:
          payload.cloudPreference === "hybrid" || payload.cloudPreference === "on-premise"
            ? "gcp"
            : payload.cloudPreference,
        description: "เครื่องบริการหลักที่ใช้ประมวลผลโลจิกธุรกิจ",
      },
      {
        id: "cache",
        name: "ระบบฐานข้อมูลชั่วคราว (Cache Layer)",
        category: "cache",
        serviceName: "Redis Cache Cluster",
        details: "High-speed in-memory database",
        status: "secure",
        provider:
          payload.cloudPreference === "hybrid" || payload.cloudPreference === "on-premise"
            ? "gcp"
            : payload.cloudPreference,
        description: "ระบบช่วยจำคำสั่งสืบค้นทั่วไปเพื่อลดการเรียกใช้ดาต้าเบสซ้ำซ้อน",
      },
      {
        id: "database",
        name: "ฐานข้อมูลประธานหลัก (Primary Database)",
        category: "database",
        serviceName: "Managed RDBMS (PostgreSQL/SQL Server)",
        details: "High Availability with Read Replicas",
        status: "secure",
        provider:
          payload.cloudPreference === "hybrid" || payload.cloudPreference === "on-premise"
            ? "gcp"
            : payload.cloudPreference,
        description: "แหล่งบันทึกข้อมูลหลักขององค์กรที่มีความสมบูรณ์สูง",
      },
      {
        id: "legacy_system",
        name: "ระบบเซิร์ฟเวอร์สำนักงานเดิม (Legacy Data Center)",
        category: "integration",
        serviceName: "On-Premises Servers",
        details: "Private Infrastructure Connected",
        status: "warning",
        provider: "on-premise",
        description: "ระบบเดิมหรือโฮสต์ภายในบริษัทที่จำเป็นต้องประสานงานข้อมูล",
      },
    ],
    connections: [
    ...
    bottlenecks: [
      {
        id: "b_1",
        title: "ความต้านทานระดับฐานข้อมูลจำกัด",
        description: "หากไม่มีระบบแคชช่วยรับแรงประทับ ทราฟฟิกอาจกดดันจนฐานข้อมูลเขียนไม่ทัน",
        severity: "high",
        solution: "ติดตั้ง Redis และเปิดระบบแยกอ่านเขียนฐานข้อมูลสำรอง (Read Replicas)",
      },
      {
        id: "b_2",
        title: "ความหน่วงของการซิงก์ข้อมูลข้ามระบบ (Hybrid Latency)",
        description: "การเชื่อมประสานงาน On-Premise และ Cloud อาจเกิดความหน่วงทางเครือข่าย",
        severity: "medium",
        solution: "ปรับรูปแบบงานเป็นแบบทำงานไม่พร้อมกัน (Asynchronous messaging via Queue/Kafka)",
      },
    ],
    securityRisks: [
      {
        id: "s_1",
        title: "ภัยคุกคามสิทธิ์เข้าใช้งานโหนดประมวลผล",
        description: "ช่องโหว่จากการเปิดพอร์ตสาธารณะหรือสิทธิ์การควบคุมระดับ IAM หละหลวม",
        severity: "high",
        mitigation:
          "ยึดหลักสิทธิ์ต่ำสุด (Least Privilege) ปิดพอร์ตเซิร์ฟเวอร์ทั้งหมดให้เข้าถึงได้เฉพาะผ่าน VPN หรือ Gateway ทันที",
      },
      {
        id: "s_2",
        title: "ความเสี่ยงภัยพิบัติข้อมูลสูญหาย",
        description: "ข้อมูลสูญหายจากเหตุการณ์ไม่คาดฝันหรือระบบฐานข้อมูลหลักขัดข้อง",
        severity: "medium",
        mitigation: "วางนโยบายทำสำเนาระบบกระจายเขตบริการ (Multi-AZ) และทดสอบกู้คืนระบบแบบรายเดือน",
      },
    ],
    costOptimization: [
      {
        item: "การใช้งานทรัพยากรแบบยืดหยุ่น (Kubernetes Auto-scaling)",
        currentEstimate: "$450 / month",
        hybridStrategy: "ตั้งค่า HPA เพื่อลดจำนวนตัวเซิร์ฟเวอร์ลง 60% ในช่วงทราฟฟิกต่ำนอกเวลาทำงาน",
        potentialSavings: "ประหยัดต้นทุน 40%",
      },
      {
        item: "ระบบจัดการตารางปิดเซิร์ฟเวอร์ทดสอบ",
        currentEstimate: "$120 / month",
        hybridStrategy:
          "กำหนดเวลาเปิด-ปิดอัตโนมัติสำหรับสภาพแวดล้อมเพื่อการพัฒนา (Dev/Test Environment)",
        potentialSavings: "ประหยัดค่าโฮสต์ 65%",
      },
    ],
    techComparison: [
      {
        category: "Compute",
        awsProduct: "Amazon ECS / EKS",
        azureProduct: "Azure AKS / Container Apps",
        gcpProduct: "Google GKE / Cloud Run",
        hybridApproach: "Kubernetes ด้วย Open-Source k3s/rancher ในศูนย์บริการบริษัท",
        pros: "ขยายตัวง่าย จัดการผ่านโค้ดสะดวก",
        cons: "มีค่าดูแลรักษาและความซับซ้อนในการควบคุมค่อนข้างสูง",
      },
      {
        category: "Database",
        awsProduct: "Amazon RDS PostgreSQL",
        azureProduct: "Azure Database for PostgreSQL",
        gcpProduct: "Google Cloud SQL PostgreSQL",
        hybridApproach: "PostgreSQL HA Cluster บนเซิร์ฟเวอร์บริษัทร่วมกับ CDC Sync",
        pros: "ดูแลรักษาง่ายโดยผู้บริการ มีระบบสำรองเรียบร้อย",
        cons: "ค่าลิขสิทธิ์และทรัพยากรระดับสูงจะมีราคาสูงขึ้น",
      },
    ],
  };
};
    ...
    const { value: validatedResult, source } = await cacheService.getOrFetch(
      ...
      async () => {
        ...
        const goalMap: Record<string, string> = {
          modernize:
            "การปรับปรุงระบบเดิมและเชื่อมต่อ Legacy ให้มีประสิทธิภาพสูงสุด (Modernize Legacy & Interoperability)",
          greenfield:
            "การวางแผนออกแบบระบบใหม่ทั้งหมดตั้งแต่ต้นให้เหมาะกับเป้าหมายธุรกิจและการเติบโต (Greenfield System Design)",
          security:
            "เน้นยกระดับความปลอดภัยและความน่าเชื่อถือสูงสุดเพื่อปกป้องทรัพย์สินดิจิทัล (Maximum Security & High Availability)",
        ...
        const riskMap: Record<string, string> = {
          ...
          strict:
            "มาตรการความปลอดภัยเข้มงวดสูงสำหรับองค์กรที่มีกฎหมายควบคุม (Highly Regulated Enterprise)",
          zero_trust:
            "การจัดการความเสี่ยงด้วยสถาปัตยกรรม Zero-Trust และแผนกู้คืนระบบระดับสูง (Full Zero-Trust & Disaster Recovery)",
        };
...
`;
        ...
        const rawResult = await executeWithRetry(async () => {
          ...
          return await withTimeout(async (signal) => {
            ...
            if (hasOutputLeakage(outputText)) {
              logger.error(
                "[OUTPUT VALIDATION CRITICAL DETECTED] Gemini response leaked instructions or secrets. Blocking output."
              );
              throw new Error(
                "ระบบตรวจพบความพยายามละเมิดนโยบายความมั่นคงปลอดภัยของซอฟต์แวร์ (Security Policy Violation Detected)"
              );
            }
        ...
        try {
          ...
          logger.info(
            "[OUTPUT VALIDATION SUCCESS] Gemini output successfully validated against ArchitectureResponseSchema."
          );
        } catch (err: any) {
          logger.error(
            "[OUTPUT VALIDATION ERROR] Gemini response did not match expected schema format. Attempting recovery.",
            err
          );
          ...
          if (safeParsed.success) {
            ...
            logger.info(
              "[OUTPUT VALIDATION RECOVERY SUCCESS] Recovered response using Zod default and safe fields."
            );
          } else {
            logger.error(
              "[OUTPUT VALIDATION CRITICAL FAILURE] Schema recovery failed. Re-throwing validation error."
            );
            throw new Error(
              "ระบบตรวจพบความผิดปกติในรูปแบบข้อมูลของปัญญาประดิษฐ์ กรุณากดปุ่มเพื่อส่งข้อมูลใหม่อีกครั้ง"
            );
          }
        }
    ...
    logger.error(
      "[CRITICAL ERROR IN GENERATING ARCHITECTURE] Falling back to structured, dynamic offline report generator due to:",
      criticalErr
    );
...
${
  currentReportObj?.nodes
    ? currentReportObj.nodes
        .map(
          (n: any) =>
            `- โหนด: ${n.name} (บริการ: ${n.serviceName}, สเปค: ${n.details}, ผู้บริการ: ${n.provider})`
        )
        .join("\n")
    : "ไม่มีข้อมูลโหนด"
}
...
`;
      ...
      logger.info(
        "Consulting Chat Advisor for client follow-up utilizing Gemini SDK Chat History Session"
      );
      ...
      return await executeWithRetry(async () => {
        ...
        return await withTimeout(async (signal) => {
          ...
          if (hasOutputLeakage(responseText)) {
            logger.error(
              "[OUTPUT VALIDATION CRITICAL DETECTED] Advisor response leaked instructions or secrets. Blocking output."
            );
